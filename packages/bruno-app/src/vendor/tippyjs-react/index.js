/**
 * Vendored copy of @tippyjs/react@4.2.6 (dist/tippy-react.esm.js, MIT).
 *
 * The published package reads `children.ref`, which React 19 moved to
 * `children.props.ref` and now logs a dev-only deprecation warning for on
 * every trigger element ("Accessing element.ref was removed in React 19").
 * Upstream is unmaintained, so imports of '@tippyjs/react' are aliased to
 * this file (rsbuild.config.mjs, jest.config.js, storybook/main.js).
 *
 * Delta from the published file: the `getElementRef` helper below, used at
 * the two former `children.ref` call sites. Everything else is unchanged.
 */
'use no memo';
...
// React 19 moved an element's ref to `element.props.ref` and warns on direct
// `element.ref` access; prefer the prop and fall back for older elements.
function getElementRef(element) {
  return element.props && 'ref' in element.props ? element.props.ref : element.ref;
}
...
function deepPreserveProps(instanceProps, componentProps) {
  let _instanceProps$popper, _componentProps$poppe;
...
let classNamePlugin = {
  ...
  fn: function fn(instance) {
    let box = instance.popper.firstElementChild;

    let isDefaultRenderFn = function isDefaultRenderFn() {
...
function TippyGenerator(tippy) {
  function Tippy(_ref) {
    ...
    let isSingletonMode = singleton !== undefined;

    let _useState = useState(false),
      mounted = _useState[0],
      setMounted = _useState[1];

    let _useState2 = useState({}),
      attrs = _useState2[0],
    ...
    let mutableBox = useMutableBox(function () {
      return {
          ...
          fn: function fn() {
            return {
              onTrigger: function onTrigger(instance, event) {
                let node = singleton.data.children.find(function (_ref2) {
                  let instance = _ref2.instance;
    ...
    let deps = [reference].concat(children ? [children.type] : []); // CREATE
    ...
    useIsomorphicLayoutEffect(function () {
      ...
      let instance = tippy(element || mutableBox.ref || ssrSafeCreateDiv(), Object.assign({}, computedProps, {
    ...
    useIsomorphicLayoutEffect(function () {
      ...
      let instance = mutableBox.instance;
        ...
        popperOptions: Object.assign({}, instance.props.popperOptions, {
          modifiers: [].concat((((_instance$props$poppe = instance.props.popperOptions) == null ? void 0 : _instance$props$poppe.modifiers) || []).filter(function (_ref3) {
            ...
            fn: function fn(_ref4) {
              ...
              let hideData = (_state$modifiersData = state.modifiersData) == null ? void 0 : _state$modifiersData.hide; // WARNING: this is a high-risk path that can cause an infinite
    ...
    return /* #__PURE__ */React.createElement(React.Fragment, null, children ? /* #__PURE__ */cloneElement(children, {
      ref: function ref(node) {
        mutableBox.ref = node;
        preserveRef(getElementRef(children), node);
      }
...
function useSingletonGenerator(createSingleton) {
  return function useSingleton(_temp) {
    ...
    useIsomorphicLayoutEffect(function () {
      ...
      let children = mutableBox.children,
      ...
      let instance = createSingleton(children.map(function (child) {
    ...
    useIsomorphicLayoutEffect(function () {
      ...
      let _sourceData$props = sourceData.props,
    ...
    return useMemo(function () {
      ...
      let target = {
        hook: function hook(data) {
          let _mutableBox$instance, _mutableBox$instance2;
...
let forwardRef = function (Tippy, defaultProps) {
...
let useSingleton = /* #__PURE__ */useSingletonGenerator(createSingleton);
let index = /* #__PURE__ */forwardRef(/* #__PURE__ */TippyGenerator(tippy));
