 ...
 *
 * 문서는 사용 모드(운영)에서도 쓸 수 있다 — 워크플로우의 동작이 아니라 설명이라
 * 운영 반영 절차를 거칠 이유가 없다. 저장하면 어디에 기록되는지는 saveNote로 알린다.
 *
 * 편집 중인지(isEditing)는 Bruno에서는 탭 상태(useDocsEditingState)에 들어 있지만,
 * flowwork는 자체 탭을 쓰므로 여기서 들고 있는다.
 */
export function DocsPane({ docs, context, emptyLabel, saveNote, autoGenerate, onSave }) {
...
더블클릭하면 바로 작성을 시작할 수 있습니다. 저장하면 즉시 기록됩니다.
`;
  ...
  return (
    <div className="wf-docs">
      ...
      <div className="wf-docs-bar">
        {saveNote ? <span className="muted hint wf-docs-note">{saveNote}</span> : null}
        {editing ? (
          <>
      ...
      <div className={`wf-docs-body ${editing ? 'editing' : ''}`}>
        <DocsEditor
          ...
          emptyPreviewContent={placeholder}
