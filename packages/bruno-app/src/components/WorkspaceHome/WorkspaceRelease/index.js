...
/**
 * 워크스페이스 → main API 반영 (웹 모드 전용).
 *
 * 이 워크스페이스 브랜치에서 작업한 API를 main과 비교해 API(파일) 단위로
 * 골라 반영하거나, main 버전으로 되돌리거나, 목록에서만 감춘다(무시).
 * main에 같은 위치·이름의 API가 독립적으로 이미 반영돼 있으면(중복)
 * 반영만 막고 원복·무시는 허용한다.
 */
const WorkspaceRelease = ({ workspace }) => {
  ...
  useEffect(() => {
    ...
    setError(null);
    serverApi
      .workspacePendingApis(workspace.name)
      .then((result) => {
        if (!alive) return;
        setPending(result);
    ...
    <StyledWrapper data-testid="workspace-release">
      <div className="release-head">
        ...
        <span className="release-desc">
          {showIgnored
            ? '무시한 API입니다. 작업 내용은 그대로 있고 목록에서만 감춰져 있습니다.'
            : '작업한 API를 선택해 main에 반영합니다. 반영된 API는 flowwork에서 바로 사용할 수 있습니다.'}
        </span>
      ...
      {pending === null ? (
        ...
        <div className="release-empty">
          {showIgnored
            ? '무시한 API가 없습니다.'
            : 'main에 반영할 API 변경이 없습니다. 컬렉션에서 API를 만들거나 수정하면 여기에 나타납니다.'}
        </div>
      ) : (
        visible.map((file) => (
          <React.Fragment key={file.path}>
            <div className={`release-row ${file.status === 'duplicate' ? 'duplicate' : ''}`}>
              ...
              {file.method && <span className={`api-method method-${file.method.toLowerCase()}`}>{file.method}</span>}
              <span className="api-name">{file.name}</span>
              <span className="api-directory" title={file.path}>
                {file.directory}
              </span>
              {file.status === 'duplicate' && (
                <span className="duplicate-hint" title="main에 같은 위치·이름의 API가 이미 있습니다">
                  중복 — 이름이나 위치를 바꿔 주세요
                </span>
              )}
              <button
      ...
      {visible.length > 0 && (
        <div className="release-footer">
          <span className="selected-count">
            {selectedPaths.length > 0 ? `${selectedPaths.length}건 선택됨` : 'API를 선택하세요'}
          </span>
          {showIgnored ? (
            <button
              ...
              onClick={() =>
                run(() => serverApi.workspaceIgnoreApis(workspace.name, selectedPaths, false), {
            ...
            <>
              <button
                ...
                onClick={() =>
                  run(() => serverApi.workspaceIgnoreApis(workspace.name, selectedPaths, true), {
              ...
              <ConfirmButton
                ...
                onConfirm={() =>
                  run(() => serverApi.workspaceRevertApis(workspace.name, selectedPaths), {
              ...
              <ConfirmButton
                ...
                title={releaseLabel}
                onConfirm={() =>
                  run(() => serverApi.workspaceReleaseApis(workspace.name, selectedPaths), {
